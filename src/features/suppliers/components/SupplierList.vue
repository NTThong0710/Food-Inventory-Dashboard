  ...
  <section class="w-full bg-[#1A2E16] text-white rounded-xl border border-[#2A362C] shadow-lg flex flex-col font-sans">
    ...
    <div class="p-4 sm:p-6 border-b border-[#2A362C] flex flex-col sm:flex-row justify-between items-center gap-4">
      <div class="flex gap-3 w-full sm:w-auto flex-wrap items-center">
        ...
        <!-- Bulk Delete Button -->
        <button v-if="selectedItems.length > 0 && authStore.hasPermission('supplier_write')" @click="$emit('delete-multiple', selectedItems)" class="flex items-center gap-2 px-4 py-2.5 bg-red-900/40 text-red-400 border border-red-900/50 rounded-lg text-sm font-medium hover:bg-red-900/60 transition-colors whitespace-nowrap">
      ...
      <div class="text-gray-400 text-sm whitespace-nowrap hidden sm:block">
        Đang hiển thị <span class="text-white font-bold">{{ paginationTextStart }}-{{ paginationTextEnd }}</span> trên <span class="text-white font-bold">{{ filteredSuppliers.length }}</span> nhà cung cấp
      </div>
    </div>

    <!-- Table Header -->
    <div class="hidden md:grid grid-cols-[40px_2.5fr_1fr_1.5fr_1fr_80px] gap-4 px-6 py-4 border-b border-[#2A362C] bg-[#152512] text-xs font-bold text-gray-500 uppercase tracking-wider items-center">
    ...
    <div class="flex flex-col">
      <div v-for="item in paginatedSuppliers" :key="item.supplier_code" 
           class="grid grid-cols-[40px_1fr] md:grid-cols-[40px_2.5fr_1fr_1.5fr_1fr_80px] gap-4 px-4 md:px-6 py-5 border-b border-[#2A362C] hover:bg-[#1A231C]/60 transition-colors items-start md:items-center group relative md:static cursor-default">
           
        <div class="flex justify-center items-center md:mt-0 md:h-full mt-[10px]">
          <input type="checkbox" v-model="selectedItems" :value="item.supplier_code" class="w-4 h-4 rounded border-gray-600 outline-none text-[#37EC13] focus:ring-1 focus:ring-[#37EC13] bg-[#1B241D] cursor-pointer accent-[#37EC13]">
        </div>
        ...
        <div class="grid grid-cols-1 md:contents gap-4 col-span-1">
          ...
          <div class="flex items-center justify-end absolute md:static right-4 top-5 md:right-0 md:top-0">
            <div class="relative" v-if="authStore.hasPermission('supplier_write')">
...
import type { Supplier } from '@/features/suppliers/store';
...
const isAllSelected = computed(() => {
  return filteredSuppliers.value.length > 0 && selectedItems.value.length === filteredSuppliers.value.length;
});
const toggleSelectAll = (e: Event) => {
  const checked = (e.target as HTMLInputElement).checked;
  if (checked) {
    selectedItems.value = filteredSuppliers.value.map(s => s.supplier_code);
  } else {
    selectedItems.value = [];
  }
};
watch(() => props.suppliers, () => {
  selectedItems.value = selectedItems.value.filter(id => props.suppliers.some(s => s.supplier_code === id));
}, { deep: true });
