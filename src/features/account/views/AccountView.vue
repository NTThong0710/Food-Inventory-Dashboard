<template>
  <div class="flex flex-col gap-6 w-full">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div class="bg-[#1A2E16] border border-[#2A362C] rounded-2xl p-6">
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-amber-50">Tiền tệ</p>
                <p class="text-xs text-gray-400">Đang dùng: <span class="text-[#37EC13] font-semibold">{{ currencyStore.currency }}</span></p>
              </div>
              <button @click="currencyStore.toggleCurrency"
                class="px-3 py-1.5 bg-[#1B5E20] text-[#37EC13] border border-[#37EC13] rounded-lg hover:bg-[#2a7c2e] transition-colors text-xs font-medium"
              >
                Chuyển sang {{ currencyStore.currency === 'VND' ? 'USD' : 'VND' }}
              </button>
            </div>
            <div class="h-px bg-[#2A362C]"></div>
            <div class="flex items-center justify-between">
    ...
    <ConfirmModal
      ...
      message="Bạn có chắc chắn muốn đăng xuất khỏi mọi thiết bị không?"
...
const updatePassword = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Thành công', detail: 'Cập nhật mật khẩu thành công', life: 3000 })
  } catch (error) {
