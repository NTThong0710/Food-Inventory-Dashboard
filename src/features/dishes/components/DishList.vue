  ...
  <section class="w-full bg-[#1A2E16] text-white rounded-xl border border-[#2A362C] shadow-lg flex flex-col font-sans">
    ...
    <div class="p-4 sm:p-6 border-b border-[#2A362C] flex flex-col sm:flex-row justify-between items-center gap-4">
      <div class="flex gap-3 w-full sm:w-auto flex-wrap items-center">
        ...
        <!-- Bulk Delete Button -->
        <button v-if="selectedItems.length > 0 && authStore.hasPermission('dish_write')" @click="$emit('delete-multiple', selectedItems)" class="flex items-center gap-2 px-4 py-2.5 bg-red-900/40 text-red-400 border border-red-900/50 rounded-lg text-sm font-medium hover:bg-red-900/60 transition-colors whitespace-nowrap">
      ...
      <div class="text-gray-400 text-sm whitespace-nowrap hidden sm:block">
        Đang hiển thị <span class="text-white font-bold">{{ paginationTextStart }}-{{ paginationTextEnd }}</span> trên <span class="text-white font-bold">{{ filteredDishes.length }}</span> món ăn
      </div>
    </div>

    <!-- Table Header -->
    <div class="hidden md:grid grid-cols-[40px_2.5fr_1.5fr_1.5fr_1.5fr_80px] gap-4 px-6 py-4 border-b border-[#2A362C] bg-[#152512] text-xs font-bold text-gray-500 uppercase tracking-wider items-center">
...
import { 
  ...
  MoreVertical, Pencil, Trash2, ChevronLeft, ChevronRight, ChefHat 
} from 'lucide-vue-next';
import type { Dish } from '@/features/dishes/store';
...
const isAllSelected = computed(() => {
  return filteredDishes.value.length > 0 && selectedItems.value.length === filteredDishes.value.length;
});
const toggleSelectAll = (e: Event) => {
  const checked = (e.target as HTMLInputElement).checked;
  if (checked) {
    selectedItems.value = filteredDishes.value.map(d => d.dish_code);
  } else {
    selectedItems.value = [];
  }
};
